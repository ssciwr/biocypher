...
neo4j:
  ...
  array_delimiter: "|"
  quote_character: "'"
  file_format: parquet # `parquet` (default, needs Neo4j >= 5.26.28) or `csv`
...
| `file_format` | File format for offline node and edge data; `"parquet"` needs Neo4j >= 5.26.28 | string | `"parquet"` (`"csv"` is also supported) |
