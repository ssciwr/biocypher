...
!!! note "Neo4j Version Compatibility"
    ...
    The exception is the default Parquet output of offline mode. The target
    Neo4j version must support import with Parquet. The version is only known
    at import time, not while BioCypher writes, so this cannot be detected for
    you; set `file_format: csv` if your target does not support Parquet import.
...
neo4j:  ### Neo4j configuration ###
  ...
  # Optional. Leave unset in the standard workflow.
  # If unset, BioCypher uses the output directory where the export files were written.
...
!!! note "Parquet is now the default"
    The target Neo4j version must support import with Parquet. The generated
    `neo4j-admin-import-call.sh` passes `--input-type=parquet`, which Neo4j
    versions without Parquet import support reject. Select CSV if your target
    does not support Parquet import, or if a downstream tool or workflow needs
    CSV data files:
...
In the standard workflow, you do not need to configure
`import_call_file_prefix`. If it is left unset, the generated import script uses
the BioCypher output directory as the file prefix for the generated headers and
data part files. Configure `import_call_file_prefix` only when the files will be
visible to `neo4j-admin` under a different path than the path where BioCypher
wrote them, for example after moving or mounting the output directory into
another environment.
